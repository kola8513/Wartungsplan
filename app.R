...
PHADIA_MONTHLY_ANCHOR <- as.Date("2026-09-11")  # Friday – Phadia monthly (4-week cycle)
ANALYZER_MONTHLY_ANCHOR <- as.Date("2026-09-01")  # Tuesday – Euroimmun Analyzer monthly (4-week cycle)
...
create_initial_table <- function(device_id = NULL) {
  ...
  if (identical(device_id, "g10")) {
    ...
    # ── Monatlich ────────────────────────────────────────────────────────────
    rows <- rbind(rows, mk_row(header = "Monatlich (Freitag, 4-wöchentlich)", task = ""))
...
und zusätzlich NORMAL/HIGH im wöchentlichen Wechsel"))
  ...
  if (identical(device_id, "g2")) {
    ...
    # ── Monatlich ────────────────────────────────────────────────────────────
    rows <- rbind(rows, mk_row(header = "Monatlich (Dienstag, 4-wöchentlich)", task = ""))
  ...
  if (identical(device_id, "g17")) {
    ...
    # ── Wöchentlich (Mittwoch) ─────────────────────────────────────────────
    rows <- rbind(rows, mk_row(header = "Wöchentlich (Mittwoch)", task = ""))
    rows <- rbind(rows, mk_row(header = "", task = "Zentrifugen: Temperatursensor säubern Bestellung Magazin"))
    ...
    rows <- rbind(rows, mk_row(header = "Druckerstatus aller Etikettendrucker 2X täglich kontrollieren (s. VA Etikettendrucker initialisieren)", task = ""))
    rows <- rbind(rows, mk_row(header = "", task = "Tagdienst (morgens)"))
    rows <- rbind(rows, mk_row(header = "", task = "Nachtdienst"))
...
load_device_table <- function(con, device_id) {
  ...
  # ---- Header text migration: Phadia / Analyzer monthly cycle names ---------
  needs_header_rename <- FALSE
  if (identical(device_id, "g10") && any(df$Header == "Monatlich (Phadia, Freitag)")) {
    df$Header[df$Header == "Monatlich (Phadia, Freitag)"] <- "Monatlich (Freitag, 4-wöchentlich)"
    needs_header_rename <- TRUE
  }
  if (identical(device_id, "g2") && any(df$Header == "Monatlich (Analyzer, Dienstag)")) {
    df$Header[df$Header == "Monatlich (Analyzer, Dienstag)"] <- "Monatlich (Dienstag, 4-wöchentlich)"
    needs_header_rename <- TRUE
  }
  if (needs_header_rename) {
    df <- order_cols(df)
    save_device_table(con, device_id, df, "<system>")
  }
      ...
      renderer = htmlwidgets::JS("
        function (instance, td, row, col, prop, value, cellProperties) {
          ...
          var map = {
            'Täglich':                       '#28a745',
            'Täglich (ZL)':                  '#28a745',
            'Täglich (Ablesen zwischen 12:00 und 14:00 Uhr)': '#28a745',
            'Montag und Donnerstag':         '#1e88e5',
            'Wöchentlich':                   '#0097a7',
            'Wöchentlich (Montag)':          '#0097a7',
            'Wöchentlich (Mittwoch)':        '#0097a7',
            'Wöchentlich (Mittwoch, TD)':    '#0097a7',
            'Wöchentlich (Donnerstag)':      '#0097a7',
            'Wöchentlich (Freitag)':         '#0097a7',
            'Wöchentlich (Freitag, ZL)':     '#0097a7',
            '14-tägig':                      '#5e35b1',
            'Monatlich':                     '#8e24aa',
            'Monatlich (Freitag, 4-wöchentlich)': '#8e24aa',
            'Monatlich (Dienstag, 4-wöchentlich)':'#8e24aa',
            'Monatlich (Freitag)':           '#8e24aa',
            'Monatlich oder alle 2500 Proben': '#8e24aa',
            'Quartalsweise':                 '#ad1457',
            'Alle 3 Monate oder alle 7500 Proben': '#ad1457',
            'Am ersten Dienstag im Monat':   '#7f0000',
            'Am ersten Freitag im Monat':    '#00695c',
            'Bei Bedarf':                    '#546e7a',
            'Wartung bei Bedarf':            '#546e7a',
            'Nach jeder Migration:':         '#546e7a',
            'Montag':                        '#bf360c',
            'Dienstag':                      '#bf360c',
            'Mittwoch':                      '#bf360c',
            'Donnerstag':                    '#bf360c',
...
server <- function(input, output, session) {
  ...
  is_header_due_today <- function(header) {
    ...
    switch(header,
           ...
           "Monatlich (Freitag, 4-wöchentlich)" = is_due_28day_cycle(Sys.Date(), PHADIA_MONTHLY_ANCHOR),
           "Monatlich (Dienstag, 4-wöchentlich)" = is_due_28day_cycle(Sys.Date(), ANALYZER_MONTHLY_ANCHOR),
  ...
  is_header_due_on <- function(header, d) {
    ...
    switch(header,
           ...
           "Monatlich (Freitag, 4-wöchentlich)" = is_due_28day_cycle(d, PHADIA_MONTHLY_ANCHOR),
           "Monatlich (Dienstag, 4-wöchentlich)" = is_due_28day_cycle(d, ANALYZER_MONTHLY_ANCHOR),
  ...
  SCHEDULE_HEADER_NAMES <- c(
    ...
    "Wöchentlich", "Wöchentlich (Montag)", "Wöchentlich (Mittwoch)",
    "Wöchentlich (Mittwoch, TD)",
    "Wöchentlich (Donnerstag)", "Wöchentlich (Freitag)",
    "Wöchentlich (Freitag, ZL)", "14-tägig",
    "Monatlich", "Monatlich (Freitag, 4-wöchentlich)",
    "Monatlich (Dienstag, 4-wöchentlich)", "Monatlich (Freitag)",
  ...
  compute_due_today <- function() {
    ...
    counts <- lapply(seq_len(nrow(devs)), function(i) {
      ...
      list(
        today     = tryCatch(as.integer(count_open_today(con, did, df_tmp)),
                             error = function(e) 0L),
        yesterday = tryCatch(as.integer(count_open_yesterday(con, did, df_tmp)),
                             error = function(e) 0L)
      )
    })
    devs$open_count           <- vapply(counts, `[[`, integer(1), "today")
    devs$open_count_yesterday <- vapply(counts, `[[`, integer(1), "yesterday")
    devs
  }
  ...
  show_due_tasks_dialog <- function() {
    ...
    if (isTRUE(already_acked)) return(invisible(NULL))
  ...
  load_device <- function(did) {
    ...
    rv$prev_task_obs_ids <- character(0)
  ...
  ensure_prev_task_observers <- function(rows) {
    for (r in rows) {
      rid <- as.character(r)
      if (rid %in% rv$prev_task_obs_ids) next
      local({
        rr <- r
        cid  <- paste0("prev_task_done_",     rr)
        oid  <- paste0("prev_task_opt_",      rr)
        rmid <- paste0("prev_task_rem_",      rr)
        nid  <- paste0("prev_task_nicht_",    rr)
        
        # Resolve the date this row's task was actually due on -- its
        # schedule's most recent due date before today, not simply
        # "yesterday" -- so saving here writes to the correct day even for
        # weekly/monthly/quarterly tasks that were missed further back.
        date_parts <- function() {
          h <- header_for_row(rr)
          d <- if (nzchar(h)) last_due_on_or_before(h, Sys.Date() - 1L) else NA
          if (is.na(d)) d <- previous_workday(Sys.Date())
          list(d = d,
        ...
        observeEvent(input[[nid]], {
          ...
          tryCatch({
            con <- pg_con(); on.exit(dbDisconnect(con), add = TRUE)
            upsert_cell(con, rv$current_device, rr, dp$day, "NE", who,
                        month = dp$month, year = dp$year)
            cur_text <- isolate(input[[rmid]]) %||% ""
            upsert_remark(con, rv$current_device, rr, dp$d,
                          cur_text, "NE", who)
  ...
  output$today_tasks <- renderUI({
    ...
    for (h in schedule_headers_present) {
      if (isTRUE(is_header_due_on(h, Sys.Date()))) next  # shown live today
      last_due <- last_due_on_or_before(h, Sys.Date() - 1L)
      if (!is.na(last_due)) due_map[[h]] <- last_due
    }
    
    overdue_missing  <- list()
    overdue_remarks_map <- list()
    if (length(due_map)) {
      my_keys <- unique(vapply(due_map, function(dd) format(dd, "%Y-%m"), character(1)))
      status_by_my <- list()
      tryCatch({
        ...
        for (h in names(due_map)) {
          due_d <- due_map[[h]]
          pr <- load_remarks_for_date(con_o, rv$current_device, due_d)
          if (is.data.frame(pr) && nrow(pr))
            for (i in seq_len(nrow(pr)))
              overdue_remarks_map[[ paste0(pr$row_index[i], "|", due_d) ]] <-
      ...
      for (rr_o in seq_len(nrows)) {
        ...
        if (!nzchar(task_o)) next
        due_d <- due_map[[h_o]]
        k     <- format(due_d, "%Y-%m")
        cs_o  <- status_by_my[[k]]
        val_o <- ""
        if (is.data.frame(cs_o) && nrow(cs_o)) {
          sub_o <- cs_o[cs_o$day == as.integer(format(due_d, "%d")) &
                          cs_o$row_index == rr_o, , drop = FALSE]
          if (nrow(sub_o)) val_o <- sub_o$value_text[1]
        }
        if (nzchar(val_o)) next
    ...
    # ── Überfällige Aufgaben banner (only when there are missing entries) ───
    if (length(overdue_missing)) {
      ensure_prev_task_observers(
        vapply(overdue_missing, function(x) as.integer(x$row), integer(1))
      )
      build_prev_row <- function(item) {
        rr_b <- item$row
        task_name_b <- if (nzchar(item$task)) item$task else paste0("Aufgabe ", rr_b)
        done_id <- paste0("prev_task_done_",     rr_b)
        opt_id  <- paste0("prev_task_opt_",      rr_b)
        rem_id  <- paste0("prev_task_rem_",      rr_b)
        nid_b   <- paste0("prev_task_nicht_",    rr_b)
        rem_val_b <- overdue_remarks_map[[ paste0(rr_b, "|", item$due) ]]$text %||% ""
        tags$div(
          class = "task-item",
          style = "border-left-color:#f57c00 !important; background:#fff8f0;",
      ...
      prev_banner_block <- tags$div(
        ...
        tags$div(
          ...
          do.call(tagList, lapply(overdue_missing, build_prev_row))
        )
      )
      ui_list <- c(list(prev_banner_block), ui_list)
    }
    ...
    for (r in seq_len(nrows)) {
      ...
      if (nzchar(hdr)) {
        ...
        sched_styles <- list(
          ...
          "Monatlich (Freitag, 4-wöchentlich)" = list("#8e24aa","#ba68c8","#4a148c", is_due_28day_cycle(Sys.Date(), PHADIA_MONTHLY_ANCHOR)),
          "Monatlich (Dienstag, 4-wöchentlich)" = list("#8e24aa","#ba68c8","#4a148c", is_due_28day_cycle(Sys.Date(), ANALYZER_MONTHLY_ANCHOR)),
        ...
        next_due_date <- switch(hdr,
                                ...
                                "Monatlich (Freitag, 4-wöchentlich)" = next_28day_due(Sys.Date(), PHADIA_MONTHLY_ANCHOR),
                                "Monatlich (Dienstag, 4-wöchentlich)" = next_28day_due(Sys.Date(), ANALYZER_MONTHLY_ANCHOR),
  ...
  schedule_description <- function(header) {
    if (is.null(header) || !nzchar(header)) return("")
    if (header %in% c("Täglich", "Täglich (ZL)", "Täglich (Ablesen zwischen 12:00 und 14:00 Uhr)"))
      return("Täglich")
    if (identical(header, "Arbeitstäglich"))
      return("Arbeitstäglich – Montag bis Freitag")
    if (header %in% c("Wöchentlich", "Wöchentlich (Montag)"))
      return("Wöchentlich – Montag")
    if (header %in% c("Wöchentlich (Mittwoch)", "Wöchentlich (Mittwoch, TD)"))
      return("Wöchentlich – Mittwoch")
    if (identical(header, "Wöchentlich (Donnerstag)"))
      return("Wöchentlich – Donnerstag")
    if (header %in% c("Wöchentlich (Freitag)", "Wöchentlich (Freitag, ZL)"))
      return("Wöchentlich – Freitag")
    if (identical(header, "Montag und Donnerstag"))
      return("Wöchentlich – Montag und Donnerstag")
    if (header %in% c("Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", "Samstag", "Sonntag"))
      return(paste("Wöchentlich –", header))
    if (identical(header, "14-tägig"))
      return("Alle 2 Wochen – Montag")
    if (identical(header, "Monatlich"))
      return("Alle 4 Wochen")
    if (identical(header, "Monatlich (Freitag, 4-wöchentlich)"))
      return("Alle 4 Wochen – Freitag")
    if (identical(header, "Monatlich (Dienstag, 4-wöchentlich)"))
      return("Alle 4 Wochen – Dienstag")
    if (identical(header, "Monatlich (Freitag)"))
      return("Monatlich – Freitag")
    if (identical(header, "Monatlich oder alle 2500 Proben"))
      return("Alle 4 Wochen oder alle 2500 Proben")
    if (identical(header, "Quartalsweise"))
      return("Alle 3 Monate")
    if (identical(header, "Alle 3 Monate oder alle 7500 Proben"))
      return("Alle 3 Monate oder alle 7500 Proben")
    if (identical(header, "Am ersten Dienstag im Monat"))
      return("Alle 4 Wochen – Dienstag")
    if (identical(header, "Am ersten Freitag im Monat"))
      return("Alle 4 Wochen – Freitag")
    if (header %in% c("Bei Bedarf", "Wartung bei Bedarf"))
      return("Bei Bedarf")
    if (identical(header, "Nach jeder Migration:"))
      return("Nach jeder Migration")
    if (identical(header, "Start-Up"))
      return("Beim Start")
    header
  }
  ...
  output$download_table_pdf <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Check if template file exists (resolve next to app.R so it works
      ...
      if (!file.exists(template_path)) {
        ...
        if (file.exists(alt)) {
          ...
          showNotification(
            paste0("Template-Datei 'wartungsplan_template.Rmd' nicht gefunden! ",
                   "Gesucht in: ", normalizePath(".", mustWork = FALSE)),
            type = "error",
            duration = 15
          )
          return()
      ...
      # Pre-flight: the template requires xelatex specifically. Checking
      # for it up front turns a missing/broken LaTeX install into a clear
      # message instead of the generic "cannot open connection" error that
      # bubbles up from deep inside rmarkdown/pandoc when the engine isn't
      # found or the compile never produces an output file.
      ...
      if (!nzchar(xelatex_path) && !tinytex_ok) {
        showNotification(
          paste0("PDF-Erstellung nicht möglich: Auf dem Server ist keine ",
                 "LaTeX-Installation (xelatex) gefunden. Bitte einen ",
                 "Administrator bitten, TinyTeX zu installieren ",
      ...
      tryCatch({
        rmarkdown::render(
          ...
          # rmarkdown writes its intermediate/knit files next to `input` by
          # default; in a deployed app that directory is often read-only,
          # which surfaces as a generic "cannot open connection" error.
          # Pointing intermediates at a writable temp dir avoids that.
        ...
        # Surface the actual render/LaTeX error so it can be diagnosed
        message("LaTeX PDF generation failed: ", e$message)
        showNotification(
          paste0("PDF-Fehler: ", e$message),
          type = "error", duration = 20
        )
  ...
  output$all_tasks_overview <- renderUI({
    req(rv$authed)
    if (!identical(rv$role, "admin")) {
      return(div(class = "alert alert-danger",
                 "Zugriff verweigert. Nur Administratoren können diese Seite öffnen."))
    ...
    devs <- devs[!(devs$device_id %in% RETIRED_DEVICE_IDS), , drop = FALSE]
    if (!nrow(devs)) {
      return(div(class = "alert alert-info", "Keine Geräte gefunden."))
    }
    
    overview_schedule_headers <- c(SCHEDULE_HEADER_NAMES, "Arbeitstäglich")
    extract_schedule_sections <- function(df) {
      headers <- if ("Header" %in% names(df)) as.character(df$Header) else character(0)
      tasks   <- if ("Task" %in% names(df)) as.character(df$Task) else character(length(headers))
      current_schedule <- ""
      current_context  <- ""
      order <- character(0)
      sections <- list()
      for (i in seq_along(headers)) {
        ...
        tsk <- tasks[i] %||% ""
        if (nzchar(hdr)) {
          if (hdr %in% overview_schedule_headers) {
            current_schedule <- hdr
            current_context  <- ""
            if (!(hdr %in% order)) order <- c(order, hdr)
            if (is.null(sections[[hdr]])) sections[[hdr]] <- character(0)
          } else {
            current_context <- hdr
          }
          next
        }
        if (!nzchar(tsk) || !nzchar(current_schedule)) next
        shown_task <- if (nzchar(current_context)) paste0(current_context, ": ", tsk) else tsk
        sections[[current_schedule]] <- c(sections[[current_schedule]], shown_task)
      }
      lapply(order, function(hdr) list(header = hdr, tasks = sections[[hdr]]))
    }
    
    device_sections <- lapply(seq_len(nrow(devs)), function(i) {
      did <- devs$device_id[i]
      label <- devs$label[i] %||% ""
      title <- if (nzchar(label)) paste0(label, " (", did, ")") else did
      df <- tryCatch(load_device_table(con, did) %||% create_initial_table(did),
                     error = function(e) create_initial_table(did))
      scheds <- extract_schedule_sections(df)
      box(
        width = 12, title = title, status = "primary", solidHeader = TRUE,
        if (!length(scheds)) {
          div(class = "alert alert-info", "Keine Aufgaben definiert.")
        } else {
          do.call(tagList, lapply(scheds, function(sec) {
            tags$div(
              style = "margin-bottom:16px;",
              tags$div(
                style = "font-weight:700; color:#003B73; font-size:15px;",
                sec$header
              ),
              tags$div(
                style = "font-size:12px; color:#666; margin:2px 0 6px 0;",
                schedule_description(sec$header)
              ),
              tags$ul(
                style = "margin-bottom:0;",
                lapply(sec$tasks, tags$li)
              )
            )
          }))
        }
    ...
    do.call(tagList, c(
      list(
        tags$div(
          ...
          style = "background: linear-gradient(135deg, #003B73 0%, #136377 100%);
                   padding:6px 12px; margin-bottom:10px; font-size:13px;",
          icon("clipboard-list"), " Alle Aufgaben sehen"
        )
      ),
      device_sections
    ))
